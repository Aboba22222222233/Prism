...
Выдели только ключевые тренды и проблемы. Максимум 3-4 предложения.`;
            ...
            const insight = await getChatResponse([{ role: 'user', content: prompt }]);
            ...
            const { error } = await supabase.from('class_insights').insert({
                class_id: id,
                content: insight,
                summary: insight.substring(0, 100) + '...' // Optional summary column
            });
    ...
    const sendMessage = async () => {
        ...
        try {
            // Context for chat
            const context = `Ты помощник учителя. Класс: ${classInfo?.name}. Ср. настроение: ${stats.avgMood}.`;
