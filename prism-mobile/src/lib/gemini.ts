...
const SITE_URL = "https://prism.app";
const SITE_NAME = "Prism Mobile";
...
export async function getChatResponse(messages: any[]) {
    ...
    for (const model of models) {
        try {
            ...
            // Enable reasoning/thinking for newer models
            if (model.includes("deepseek") || model.includes("gpt-oss")) {
                // Some providers might need specific params, keeping simple for now
            }
            ...
            console.warn(`Chat Model ${model} error:`, error);
        }
    ...
    throw new Error("Все AI сервисы недоступны.");
}
